...
- **Signaux Règles & ML** : `regle_rib_partage`, `regle_montant_anormal`, `regle_doublon_facture`, `regle_creation_tardive`, `prediction_ml` (Isolation Forest, `n_estimators=1000` : -1 = Anomalie, 0/1 = Normal).
...
- **ML & Règles (Niveaux 1-2)** : Détectent très bien les fraudes transactionnelles isolées. Le modèle Isolation Forest final (1000 arbres) capte ~34% des fraudes multi-signaux, et la combinaison avec les règles atteint un **taux de détection global de 72.4%** sur le jeu de référence. Ils échouent logiquement sur les cas de collusion purs (0/8 détectés).
