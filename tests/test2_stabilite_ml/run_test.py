...
# Appliquer regles (car ml_features peut s'attendre a certaines colonnes bien que pas les features)
# Mais les features utilisees ne dependent pas des regles!
...
for seed in seeds:
    ...
    subprocess.run([
        ...
        "--output", os.path.join(ROOT_DIR, f"tests/test2_stabilite_ml/transactions_scorees_{seed}.csv")
    ], stdout=subprocess.DEVNULL)
    
    # Eval
    j, t = load_data(os.path.join(ROOT_DIR, "data/raw/journal_fraudes_injectees.csv"), os.path.join(ROOT_DIR, f"tests/test2_stabilite_ml/transactions_scorees_{seed}.csv"))
    metrics = evaluate_exact(j, t)
    
    ml_classique_num, ml_classique_den = metrics["ml"]["classique"]
    ...
    print(f"Rappel Global: {rappel_global:.1f}% | Precision: {precision:.1f}% | Alertes: {total_alertes}")

df_res = pd.DataFrame(results)
print("\n=== SYNTHESE ===")
print(df_res)
df_res.to_csv(os.path.join(ROOT_DIR, "tests/test2_stabilite_ml/resultats.csv"), index=False)
